...
I would add Testcontainers coverage against MySQL and OpenAPI documentation first. H2 MySQL mode is
useful for fast tests, but a real MySQL container catches dialect-specific migration issues before
deployment.
...
operational metrics, and fails the build if service coverage falls below the agreed threshold.
